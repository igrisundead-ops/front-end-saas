  ...
  return (
    <ChatInputContext.Provider value={contextValue}>
      <div
        className={cn(
          variant === "default" &&
            "flex flex-col items-end w-full p-2 rounded-2xl border border-white/20 bg-white/5 backdrop-blur-sm focus-within:ring-1 focus-within:ring-white/30 focus-within:outline-none",
...
function ChatInputTextArea({
  ...
  return (
    <Textarea
      ...
      className={cn(
        "max-h-[400px] min-h-0 resize-none overflow-x-hidden text-white placeholder:text-white/40",
        variant === "unstyled" &&
...
function ChatInputSubmit({
  ...
  if (loading && onStop) {
    return (
      <Button
        onClick={onStop}
        className={cn(
          "shrink-0 rounded-full p-1.5 h-fit border border-white/30 bg-white/10 hover:bg-white/20 text-white",
          className,
        )}
        {...props}
      >
  ...
  return (
    <Button
      className={cn(
        "shrink-0 rounded-full p-1.5 h-fit border border-white/30 bg-white/10 hover:bg-white/20 disabled:opacity-50 text-white",
        className,
      )}
      disabled={isDisabled}
      onClick={(event) => {
